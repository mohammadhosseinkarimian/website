// DeckClubs.jsx — fan deck (Clubs), mirrors others
...
const clubFour = [
  { rank: "A",  title: "Flora", caption: "Developer" },
  { rank: "K",  title: "Tour Guide",     caption: "Scrum Master" },
  { rank: "Q",  title: "MHQ",      caption: "Developer" },
  { rank: "J",  title: "BoardGame",   caption: "Front-End Dev" },
];

export default function DeckClubs({ items = clubFour, routeForItem = () => "/software" }) {
        ...
        {items.map((it, i) => {
          ...
          return (
            <article
              ...
              title={`${it.rank} of Clubs — ${it.title}`}
            >
              <CardClub rank={it.rank} title={it.title} caption={it.caption} />
            </article>
          );
        })}
      </div>
      ...
      {active && (
        <div ref={portalRef} className="deck-portal show" onClick={closeExpanded}>
          ...
          <div className="deck-ghost" ref={ghostRef} onClick={(e) => e.stopPropagation()}>
            <CardClub fill rank={active.item.rank} title={active.item.title} caption={active.item.caption} />
          </div>
