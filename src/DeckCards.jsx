// DeckCards.jsx — fan deck (Spades), uses Card.jsx; includes takeover → loader → veil → router
import React, { useEffect, useRef, useState } from "react";
import { useNavigate } from "react-router-dom";
import Card from "./Card";

/* ===== timings (tweak OPEN_MS for takeover speed) ===== */
const OPEN_MS = 1400;
const CLOSE_MS = 900;
...
if (w < 640) return { 
  ...
  useEffect(() => {
    const onR = () => setCfg(calc(window.innerWidth)); // window.innerWidth (fix)
...
/* ===== body-level veil (stays across routes) ===== */
...
/* ===== default items (A♠ K♠ Q♠ J♠ 10♠) ===== */
const spadeFive = [
  { rank: "A",  title: "Web Designer",      caption: "Asim" },
  { rank: "K",  title: "Research Assistant", caption: "University of Regina" },
  { rank: "Q",  title: "ML Intern",       caption: "K. N. Toosi" },
  { rank: "J",  title: "Scrum Master",    caption: "IUST" },
  { rank: "10", title: "Lab Instructor",        caption: "University of Regina and IUST" },
];

export default function DeckCards({ items = spadeFive, routeForItem = () => "/software" }) {
  ...
  /* scroll-bounce when entering viewport */
  useEffect(() => {
  ...
  function openExpanded(index, slotEl) {
    ...
    const cardEl = slotEl.querySelector(".deck-card"); // from Card.jsx wrapper
    ...
    requestAnimationFrame(() => {
      ...
      // Center scaling to keep proportions during takeover
      ...
      requestAnimationFrame(() => {
        ...
        const finish = () => {
          ...
          // snapshot the painterly canvas to hide the deck under loader
          ...
          // after loader, fade with route veil and navigate
          setTimeout(() => {
        ...
        {items.map((it, i) => {
          ...
          return (
            <article
              ...
              title={`${it.rank} of Spades — ${it.title}`}
            >
              <Card rank={it.rank} title={it.title} caption={it.caption} />
            </article>
          );
        })}
      </div>
      ...
      {active && (
        <div ref={portalRef} className="deck-portal show" onClick={closeExpanded}>
          ...
          <div className="deck-ghost" ref={ghostRef} onClick={(e) => e.stopPropagation()}>
            {/* IMPORTANT: ghost fills its container to avoid half-screen issue */}
            <Card fill rank={active.item.rank} title={active.item.title} caption={active.item.caption} />
          </div>
