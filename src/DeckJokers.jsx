// DeckJokers.jsx — fan deck with two Jokers (education)
...
function useDeckLayout() {
  ...
  deckH: "min(48vh, 380px)",   // was min(64vh,520px)
  cardW: "min(50vw, 240px)"    // was min(82vw,340px)
};
if (w < 640) return { 
  spreadDeg: 16, 
  spreadScale: 0.5,           // was 0.85
  deckH: "min(52vh, 440px)",   // was min(64vh,540px)
  cardW: "min(52vw, 270px)"    // was min(68vw,340px)
};if (w < 900) return { spreadDeg: 22, spreadScale: 0.67, deckH: "clamp(280px,50vw,480px)", cardW: "clamp(220px,28vw,300px)" };
    return              { spreadDeg: 10, spreadScale: 0.8, deckH: "clamp(300px,38vw,440px)", cardW: "clamp(230px,24vw,320px)" };
  };
...
const jokerTwo = [
  { title: "Education, Master's of CS", caption: "University of Regina" },
  { title: "Education, Bachelor's of CS", caption: "IUST" },
];

export default function DeckJokers({ items = jokerTwo, routeForItem = () => "/software" }) {
        ...
        {items.map((it, i) => {
          ...
          return (
            <article
              ...
              title={`Joker — ${it.title}`}
            >
              <CardJoker title={it.title} caption={it.caption} />
            </article>
          );
        })}
      </div>
      ...
      {active && (
        <div ref={portalRef} className="deck-portal show" onClick={closeExpanded}>
          ...
          <div className="deck-ghost" ref={ghostRef} onClick={(e) => e.stopPropagation()}>
            <CardJoker fill title={active.item.title} caption={active.item.caption} />
          </div>
