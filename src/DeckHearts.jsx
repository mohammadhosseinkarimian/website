// DeckHearts.jsx — fan deck (Hearts), mirrors DeckCards.jsx but uses CardHeart
import React, { useEffect, useRef, useState } from "react";
import { useNavigate } from "react-router-dom";
import CardHeart from "./CardHeart";

/* ===== timings (same as spades) ===== */
const OPEN_MS = 1400;
const CLOSE_MS = 900;
...
/* ===== body-level veil (reuse from spades version) ===== */
...
const heartFive = [
  { rank: "A",  title: "Image Processing",      caption: "ML Intern" },
  { rank: "K",  title: "RL in OpenAI GYM ", caption: "RL Engineer" },
  { rank: "Q",  title: "Sustainability",       caption: "ML Engineer" },
  { rank: "J",  title: "Emotion Detection",    caption: "NLP Engineer" },
  { rank: "10", title: "MBTI Detector",        caption: "ML Engineer" },
];

export default function DeckHearts({ items = heartFive, routeForItem = () => "/software" }) {
        ...
        {items.map((it, i) => {
          ...
          return (
            <article
              ...
              title={`${it.rank} of Hearts — ${it.title}`}
            >
              <CardHeart rank={it.rank} title={it.title} caption={it.caption} />
            </article>
          );
        })}
      </div>
      ...
      {active && (
        <div ref={portalRef} className="deck-portal show" onClick={closeExpanded}>
          ...
          <div className="deck-ghost" ref={ghostRef} onClick={(e) => e.stopPropagation()}>
            ...
            <CardHeart fill rank={active.item.rank} title={active.item.title} caption={active.item.caption} />
          </div>
