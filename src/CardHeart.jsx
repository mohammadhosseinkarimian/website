// CardHeart.jsx — reusable playing card (Hearts face), theme-aware luxe styling
...
/* ===== curvy ♥ with soft bevel, inner etch & sparkle ===== */
function TrueHeart({ outline = foil() }) {
  return (
    <svg viewBox="0 0 100 120" width="100%" height="100%" style={{ display: "block" }} aria-hidden>
      <defs>
        {/* bevel sheen */}
        <radialGradient id="rgHeartSheen" cx="50%" cy="30%" r="70%">
          <stop offset="0%"  stopColor={toneHi(20)} stopOpacity=".9" />
          <stop offset="50%" stopColor={toneHi(14)} stopOpacity=".18" />
          <stop offset="100%" stopColor={toneHi(8)}  stopOpacity="0" />
        </radialGradient>
        {/* delicate hatch for engraving */}
        <pattern id="ptHeartEtch" width="6" height="6" patternUnits="userSpaceOnUse" patternTransform="rotate(24)">
          <line x1="0" y1="0" x2="0" y2="6" stroke={toneHi(8)} strokeOpacity=".16" strokeWidth="0.6" />
        </pattern>
        <filter id="fSoftHeart"><feGaussianBlur in="SourceGraphic" stdDeviation="0.5" /></filter>
      </defs>
      ...
      <path
        d="M50 22
           ...
           L 78 60
           C 92 46, 94 23, 78 12
           C 67 4, 51 11, 50 22 Z"
        fill="url(#rgHeartSheen)"
        stroke={outline}
        strokeWidth="2"
        strokeLinejoin="round"
        filter="url(#fSoftHeart)"
      />
      ...
      <path
        d="M50 26
           ...
           L 75 56
           C 87 45, 88 26, 75 16
           C 65 9, 51 16, 50 26 Z"
        fill="url(#ptHeartEtch)"
        opacity=".32"
        style={{ mixBlendMode: "screen" }}
      />

      {/* subtle glint on the left lobe */}
      <path
        d="M30 28 C 26 32, 26 39, 31 43"
        fill="none"
        stroke={toneHi(22)}
        strokeOpacity=".6"
        strokeWidth="1.1"
      ...
      {/* tiny sparkles */}
      <circle cx="63" cy="28" r="1.4" fill={toneHi(22)} opacity=".8"/>
      <circle cx="67" cy="33" r="1.0" fill={toneHi(22)} opacity=".65"/>
    </svg>
  );
}
...
/* ===== PUBLIC: CardHeart ===== */
export default function CardHeart({ rank = "A", title, caption, fill = false }) {
